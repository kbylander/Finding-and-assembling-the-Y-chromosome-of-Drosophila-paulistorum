...
data=pd.read_csv("average_F_M_depth_filt_male0.out", sep='\t',names=colnames)
...
#for every instans, if the mean ratio is 0 and mixed > 0.5, save the contig and add 1 to corresponding index in frequency.
for i in range(0,len(data["contig"])):
    if (data["average ratio"][i] == 0 and data["sum mixed"][i] > 0.5):
...
#Upload as csv
put_cont.to_csv("./putative_contigs_mix_0_05.csv", sep='\t', index=False, header=False, encoding='utf-8')
