...
$ARGV[0] =~/(\S+).out/ || die;
my $name=$1;
#the new output file name as *_chang.out
open (OUT,">$1_chang.out");
while(my $line=<input_file>){
	my @linearray=split(/\s+/,$line);
	#filtered the region with 1 or less depth 
	next if ($linearray[2]<=1);
	#put the median into a array 
	$ratio[$i]=$linearray[2]; #CHANGED TO ONLY VALUE, NOT RATIO BETWEEN VALUES
	#sum of depth in two different samples
	$sum=$sum+$linearray[2];
	# $sum2=$sum2+$linearray[3]; #REMOVE
	$i++;
	...
	if((($linearray[1] >=10000*($j+1)  || $linearray[0] ne $name) && $start !=1 )){
		...
		print OUT "$name\t$base\t$i\t$sum\t$ave\t$med\n"; #REMOVE SUM2
		$j++;
		$i=0;
		$sum=0;
		# $sum2=0; #REMOVE
